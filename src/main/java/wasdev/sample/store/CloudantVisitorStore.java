...
public class CloudantVisitorStore implements MedicalStore {
	...
	@Override
	public Collection<Medical> getAll(){
        List<Medical> docs;
		try {
			docs = db.getAllDocsRequestBuilder().includeDocs(true).build().getResponse().getDocsAs(Medical.class);
		} catch (IOException e) {
			return null;
		}
        return docs;
	...
	public Medical get(String id) {
		return db.find(Medical.class, id);
	}

	@Override
	public Medical persist(Medical me) {
		String id = db.save(me).getId();
		return db.find(Medical.class, id);
	}

	@Override
	public Medical update(String id, Medical newMedical) {
		Medical medical = db.find(Medical.class, id);
		medical.setName(newMedical.getName());
		db.update(medical);
		return db.find(Medical.class, id);
